...
<body>
    ...
    <script>
        ...
        async function loadAllData() {
            try {
                const [reservations, stations, locations] = await Promise.all([
                    authenticatedFetch(API_BASE_URL + '/api/reservations').then(r => r.json()),
                    authenticatedFetch(API_BASE_URL + '/api/stations').then(r => r.json()),
                    authenticatedFetch(API_BASE_URL + '/api/locations').then(r => r.json())
                ]);
        ...
        function loadCurrentReservations() {
            ...
            // Réservations en cours = PENDING, CONFIRMED, ACTIVE ou CANCELLED dont la date n'est pas encore passée
            const now = new Date();
            const current = allReservations.filter(r => {
                ...
                // CANCELLED seulement si la date de fin n'est pas encore passée
            ...
            body.innerHTML = current.map(reservation => {
                return '<tr>' +
                    ...
                    '<td>' + (reservation.totalAmount ? reservation.totalAmount.toFixed(2) : '0.00') + ' €</td>' +
        ...
        function loadPastReservations() {
            ...
            // Réservations passées = COMPLETED, REFUSED, ou CANCELLED dont la date est passée
            const now = new Date();
            const past = allReservations.filter(r => {
                if (r.userId !== CURRENT_USER_ID) return false;
                
                // COMPLETED et REFUSED sont toujours dans les réservations passées
                if (r.status === 'COMPLETED' || r.status === 'REFUSED') return true;
                
                // CANCELLED seulement si la date de fin est passée
                if (r.status === 'CANCELLED') {
                    const endTime = new Date(r.endTime);
                    return endTime < now;
                }
        ...
        async function acceptReservation(id) {
            try {
                const response = await authenticatedFetch(API_BASE_URL + '/api/reservations/' + id + '/confirm', {
                    method: 'PUT'
                });
        ...
        async function refuseReservation(id) {
            ...
            try {
                const response = await authenticatedFetch(API_BASE_URL + '/api/reservations/' + id + '/refuse', {
                    method: 'PUT'
                });
                if (!response.ok) throw new Error('Erreur');
        ...
        async function cancelReservation(id) {
            ...
            try {
                const response = await authenticatedFetch(API_BASE_URL + '/api/reservations/' + id + '/cancel', {
                    method: 'PUT'
                });
                if (!response.ok) throw new Error('Erreur');
        ...
        async function deleteLocation(id) {
            ...
            try {
                const response = await authenticatedFetch(API_BASE_URL + '/api/locations/' + id, {
                    method: 'DELETE'
                });
                if (!response.ok) throw new Error('Erreur');
