...
<body>
    ...
    <script>
        ...
        async function loadPlugTypes() {
            try {
                const response = await fetch(API_BASE_URL + '/api/plug-types');
                if (!response.ok) {
        ...
        async function handleSubmit(event) {
            ...
            try {
                // Utiliser authenticatedFetch au lieu de fetch simple pour inclure le token JWT
                const response = await authenticatedFetch(API_BASE_URL + '/api/vehicles?userId=' + CURRENT_USER_ID, {
                    method: 'POST',
